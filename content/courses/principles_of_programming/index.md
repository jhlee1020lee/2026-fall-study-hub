---
title: 프로그래밍의 원리
description: 프로그래밍의 원리 강의 흐름, 개념 연결, 복습 문제
tags: [course, principles-of-programming]
---

# 프로그래밍의 원리

## 강의노트

- [[courses/principles_of_programming/lectures/2026-09-01-lecture-01|2026-09-01 · 1강]]
- [[courses/principles_of_programming/lectures/2026-09-03-lecture-02|2026-09-03 · 2강]]

## 개념 지도

- [[concepts/formal-verification|formal verification(형식 검증)]]
- [[concepts/abstraction-level|abstraction level(추상화 수준)]]
- [[concepts/proof-checking|proof checking(증명 검사)]]
- [[concepts/runtime-monitor|runtime monitor(실시간 모니터)]]
- [[concepts/specification|specification(명세)]]
- [[concepts/추상화|Abstraction(추상화)]]
- [[concepts/추상-구문-트리|Abstract Syntax Tree(AST; 추상 구문 트리)]]
- [[concepts/코드-재사용|Code Reuse(코드 재사용)]]
- [[concepts/조건식|Conditional Expression(조건식)]]
- [[concepts/평가|Evaluation(평가)]]
- [[concepts/평가-전략|Evaluation Strategy(평가 전략)]]
- [[concepts/함수-적용|Function Application(함수 적용)]]
- [[concepts/함수형-프로그래밍|Functional Programming(함수형 프로그래밍)]]
- [[concepts/명령형-프로그래밍|Imperative Programming(명령형 프로그래밍)]]
- [[concepts/반복문|Loop(반복문)]]
- [[concepts/이름-바인딩|Name Binding(이름 바인딩)]]
- [[concepts/객체지향-프로그래밍|Object-Oriented Programming(객체지향 프로그래밍)]]
- [[concepts/소유권-타입|Ownership Type(소유권 타입)]]
- [[concepts/재귀|Recursion(재귀)]]
- [[concepts/치환|Substitution(치환)]]
- [[concepts/종료와-발산|Termination and Divergence(종료와 발산)]]
- [[concepts/타입|Type(타입)]]
- [[concepts/타입-클래스|Type Class(타입 클래스)]]
- [[concepts/값과-식|Value and Expression(값과 식)]]

## 강의자료

- [[page_cache/principles_of_programming/lecture-part1/page-001|Part 1 페이지 미리보기]]

현재 페이지 미리보기만 제공하며, PDF 원본 파일은 이 사이트에 별도로 업로드하지 않았습니다.
